...
/** helper to get clustering-cloud out of given poi-data (ensure to have latitude/longitude attribtes present as double values) */
var ClusterHelper = {
	...
	/** caluclates center/average point of given pois, ensure pois have .latitude and .longitude double values */
	...
	toMercator: function(latitude, longitude) {
	...
	toLatLon: function(mercX,mercY) {
	    ...
	    var longitude    = mercX / shift * 180.0;
	    var latitude    = mercY / shift * 180.0;
	...
	* usrLocation: e.g. { 'latitude': 47.7732725, 'longitude': 13.0695639 }
	* array of places, whereat every poi must hat latitude & longitude attribute (as in usrLocation)
	*
	* returns array of clustered places, each having "latitude", "longitude", "places" (as defined in placesArray, lenght = 1 if type is 'poi', length>1 if 'cluster') and "type" ('poi' or 'cluster') attribute
	**/
    ...
    for (var i=0; i<placesArray.length; i++) {
      ...
      var poi = placesArray[i].poi;

      /* angle between user and place */
      var angle = Math.round(ClusterHelper.angleBetween(usrLocation.latitude, usrLocation.longitude, poi.latitude, poi.longitude));
      ...
      var clusterValue = null;
        ...
        if (typeof(cluster) !== 'undefined' && cluster != null) {
          ...
          if (cluster.length>0) {
          	...
          	/* in case there is only one poi present -> use original latitude/longitude of poi */
            if (cluster.length==1) {
              clusterValue = {'latitude': cluster[0].latitude, 'longitude': cluster[0].longitude, 'places': cluster, 'type': 'poi'};
            }
            /* otherwise calculate center poi out of clustered pois and store pois in "places" attribute */
              ...
              clusterValue = {'latitude': centerPoint.latitude, 'longitude': centerPoint.longitude, 'places': cluster, 'type': 'cluster'};
            } 
